...
- **You want to leave it running all day unwatched.** Our kit makes agent-flow refuse requests from other websites, but any program already running on your own computer can still open the page and read your conversation while agent-flow runs. Stop it when you are not watching: `python start.py --stop`.
...
- **Windows fix 1, the launch folder.** The server remembers the folder it was started from, and only accepts events from Claude Code sessions running inside that folder. Started from the wrong place, it shows nothing. So it was set to start from the Documents folder, which contains every vault.
- **Start by itself when the computer starts, with no window.** A small Windows script file, `agent-flow.vbs`, went into the Windows Startup folder (Windows runs everything in that folder each time you switch on your computer and sign in to Windows). It runs `npx -y agent-flow-app --no-open`, and `--no-open` stops it opening a browser tab.
...
> **Warning:** 2 limits remain. First, the checks work only when our kit starts agent-flow (`python start.py`, or our file that starts it when the computer starts). A copy started by hand with `npx agent-flow-app` has none of them. Second, any program already running on your own computer can still open the page, as with every page at 127.0.0.1. If you are not watching agent-flow, stop it with `python start.py --stop`.
...
- A small file in your Startup folder that starts agent-flow by itself when the computer starts, with no window, from the folder that contains your vaults, with usage tracking off.
...
- If Windows hands agent-flow's private port to another program in the moment before agent-flow takes it, `guard.py` picks another number and tries again, up to 3 times (1 start in 34 failed this way on 2026-09-22).
...
**On a Mac:** a current Mac has a command called `python3` and none called `python`. Wherever this guide says `python`, type `python3`, and where it says `python -m pip`, type `python3 -m pip`. Paths in this guide are written the Windows way; on a Mac, your home folder is `~`, so `C:\Users\<you>\.claude` is `~/.claude`.

| You need | How to check | If it is missing |
|---|---|---|
| Claude Code | `claude --version` | You have it from earlier Outliers Accelerator sessions. |
| Python 3.11 or newer | `python --version` (Mac: `python3 --version`) | python.org. `install.py` tests this and stops if it is older, because Python 3.8 stopped getting security fixes on 2024-10-07 and Python 3.10 gets them only until 2026-10-31. |
...
```
   ...
   On a Mac use `python3`. Keep the folder where it lands: the small file that starts agent-flow by itself when the computer starts points at it.
3. Answer 3 questions. Press Enter to accept the suggestion in square brackets.
   ...
   If a question shows no suggestion, paste the full path of that vault (Windows: in File Explorer, click the address bar and copy it. Mac: in Finder, right-click the folder, press the Option key, and choose Copy as Pathname while it is down). No CRM vault yet? Press Enter to leave it blank.
...
5. Read the table the installer prints at the end: 1 row per kind of event. Every event should show `1` in the "after" column. If you had old copies, the "before" column shows how many, and a dated backup of your settings is named on screen. The file that starts agent-flow when the computer starts is `outliers-agent-flow.vbs` in your Startup folder; that is the name you see in Windows' Startup apps list.

![What a successful install prints, retaken 2026-09-24 with the 2 version checks it now runs first. Paths shortened to C:\Users\<you>; the demo ran in a made-up home folder, and "Demo CRM" is an invented vault.](img/install-output.png)

6. Start it now: `python start.py`. From now on it also starts by itself, with no window, each time you switch on your computer and sign in.
...
> **Tip:** to remove everything later, run `python install.py --uninstall`. It stops the server, takes a backup, removes every agent-flow hook entry, and deletes the file that starts agent-flow when the computer starts. Your settings go back to exactly how they were, character for character, as long as the backup from your first install is still there. If you had no settings file before, an empty settings file (containing only `{}`) is left. The folder `.claude\agent-flow` stays; you may delete it by hand.
...
![Asking whether agent-flow is running, stopping it, then trying to stop it a second time, as it prints.](img/status-stop.png)
...
**The safe way.** Make your changes in a copy of the folder, called `agent-flow-test`, never in the folder you installed. In the copy, run only the self-checks. Never run `install.py` or `start.py` in the copy: both act on the same settings file, the same file that starts agent-flow when the computer starts, and the same running agent-flow as your everyday folder, so the copy would take them over. The self-checks are safe while your everyday agent-flow is running: they use a made-up home folder and a stand-in for agent-flow, never your real settings or port 3001.
...
Ashley changed his own copy. He tried patching agent-flow to put every session in 1 tab, looked at the screen, saw 6 sessions stacked on the same spot, and threw the patch away. He worked out the 2 Windows fixes himself: start the server from the folder that sits above all your vaults, or it never hears your sessions; and delete the leftover registration files, or events stop arriving with no warning. His own file in the Windows Startup folder, which started agent-flow with no window each time the computer started, left usage tracking on; ours switches it off. He added a count of hooks after his own settings file reached 12 copies of the same hook on every event. The download you have contains his 2 Windows fixes and the hook count.

Each idea below comes with a prompt you can paste into Claude Code, opened in the `outliers-ws-01-agent-flow` folder unless it says otherwise: in a new terminal, type `cd outliers-ws-01-agent-flow`, then `claude`.
...
4. **Give every subagent a real name.** If your session starts a subagent that is not one of the named agents in your agents folder (the `.claude\agents` folder inside your home folder, where each agent has its own file), it is labelled with the first words of the job it was given, such as "Read all ...", instead of an agent's name. Named agents make the picture readable.
...
| `--foreground`, `--quiet` | Used by the files that start agent-flow when the computer starts. `--foreground`: stay running until agent-flow stops, instead of handing your terminal straight back, which the Mac start-up job needs. `--quiet`: print nothing at all, which the Windows file needs. |
...
- The file that starts agent-flow when the computer starts: on Windows `outliers-agent-flow.vbs` in your Startup folder; on a Mac a start-up job, `~/Library/LaunchAgents/com.outliers.agent-flow.plist`, which runs the next time you sign in to your Mac (to start it now: `launchctl load` followed by that path).
...
| The page never loads after you switch the computer on | The settings check refused to start agent-flow when the computer started, or the start failed, and nothing was on screen to tell you. | `python start.py --status` prints the 1-sentence reason; fix as it says, then `python start.py`. |
...
![A settings file with a comma after its last item: the start is refused, `--status` repeats the reason, and `check_hooks.py` says the same words. The last part of the picture is an empty settings.json file, which `start.py` and `check_hooks.py` used to describe differently until 2026-09-23. Made-up home folder, 2026-09-23.](img/start-refused.png)

![A start that failed, saying why in 1 sentence instead of pointing at a log. We made this failure happen on purpose on 2026-09-23 by starting another program on port 3001 first; the words are the ones our scripts really print.](img/start-failed-says-why.png)

![Closing agent-flow by force leaves a second server on the same folder. `cleanup.py` counts how many agent-flow servers are running, instead of how many folders are being watched, ends the older server, and the next start prints a line saying it did. Made-up home folder, 2026-09-23.](img/leftover-server-ended.png)
